...
public class SearchComponents : EditorWindow
{
    [MenuItem("Custom/Search Components")]
    static void Init()
    {
        SearchComponents window = (SearchComponents)EditorWindow.GetWindow(typeof(SearchComponents));
        window.Show();
        window.position = new Rect(20, 80, 550, 500);
    }

    string[] modes = new string[] { "Search usage components ", "Search missing components" };
    ...
    void OnGUI()
    {
        ...
        Rect windowRect = GUILayoutUtility.GetRect(1, 25);
        windowRect.x += 4;
        windowRect.width -= 7;
